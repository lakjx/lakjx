        ...
        if self.current_transmitting is None:  # 只有当前没有传输时才开始新的传输
            for i, ue in enumerate(self.ues):
                if ue.transmit_sdu(current_time):
                    ...
                    ue.phy_act = 'Transmit'
                    if np.random.random() < 0.9999:  # 传输成功率0.9
                        ue.dcm_act = 'ACK'
                        ue.suceess = True
                    break
...
def collect_datasets(ue_nums=[2,3,4,5,6,8],pa=[0.5,0.41,0.33,0.25,0.16,0.083],buffer_sizes=20,steps=35,episode=300):
    dataset = []
            ...
            for _ in range(episode):
                ...
                for step in range(steps):
                    for ue in bs.ues:
                        ...
                        ue.dcm_act = None
                        ue.phy_act = None
                        if np.random.random() < bs.arrival_prob and step != 0:
                            ue.add_sdu()
                    bs.UE_obs = np.array([len(ue.buffer) for ue in bs.ues])
...
if __name__ == "__main__":
    ...
    # with open('./data/raw_datasets2.json', 'w') as f:
    #     json.dump(dataset, f, indent=4)

    #raw_dataset2 是UE=2,3,4,5,6  已更新 time_step=0时不产生SDU
    #raw_dataset3 是UE=2,5  未更新
    #raw_dataset4 是UE=2,5,8 & p=0.5,0.41,0.33,0.25,0.16,0.083  已修正UEx is tx的问题