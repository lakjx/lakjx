...
class MacProtocolEnv():
    ...
    def reset(self):
        ...
        self.BS_obs = -1 * np.ones((1,), dtype=np.int32)
        ...
        return obs_n
    # def cont_free_step(self,action_n,UCM,DCM):
    #     #如果UCM为None，则随机生成UCM
    #     if UCM is None:
    #         UCM = np.zeros(self.UE_num, dtype=np.int32)
    #         DCM = np.zeros(self.UE_num, dtype=np.int32)
    #     UE_actions = action_n
    #     #随机生成UE的SDU
    #     for UE in self.UEs:
    ...
    #     #UE端
    #     sr_que,ue_ucm,ue_act = [],[],[]
    #     for UE in self.UEs:
    #         if len(UE.buff) > 0:
    #             sr_que.append(UE.name)
    #             ue_ucm.append(self.UE_msg_space['SR'])
    #         else:
    #             ue_ucm.append(self.UE_msg_space['None'])
            
    #         if DCM[UE.name_id] == self.BS_msg_space['SG'] and len(UE.buff) > 0:
    #             ue_act.append(self.UE_act_space['Transmit'])
    #         elif DCM[UE.name_id] == self.BS_msg_space['ACK'] and len(UE.buff) > 0:
    #             ue_act.append(self.UE_act_space['Delete'])
    #         else:
    #             ue_act.append(self.UE_act_space['Do Nothing'])
    #     #BS端
    #     if sr_que:
    #         bs_dcm = []
    #         selected_ue = np.random.choice(sr_que)
    #         for UE in self.UEs:
    #             if UE.name == selected_ue:
    #                 if UE_actions[UE.name_id] == self.UE_act_space['Transmit']:
    #                     bs_dcm.append(self.BS_msg_space['ACK'])     
    #                 else:
    #                     bs_dcm.append(self.BS_msg_space['SG'])
    #             else:
    #                 bs_dcm.append(self.BS_msg_space['None'])
    #     else:
    #         bs_dcm = [self.BS_msg_space['None']]*self.UE_num
        
    #     #执行动作
    #     for id,ue in enumerate(self.UEs):
    #         if UE_actions[id] == self.UE_act_space['Transmit']:
    #             data = ue.transmit_SDU()
    #             if data is not None:
    #                 self.data_channel.append(data)
    #         elif UE_actions[id] == self.UE_act_space['Delete']:
    #             del_data = ue.delete_SDU()

    #     self.check_channel(error_del=0)
    ...
    #     return np.array(ue_act,dtype=np.int32),np.array(ue_ucm,dtype=np.int32),np.array(bs_dcm,dtype=np.int32)
        
    def cont_free_step(self, action_n, UCM, DCM):
        # 如果UCM为None，则初始化UCM和DCM
        if UCM is None:
            UCM = np.zeros(self.UE_num, dtype=np.int32)
            DCM = np.zeros(self.UE_num, dtype=np.int32)
        ...
        # 检查是否有UE正在传输数据
        transmitting = False
        transmitting_ue = None
        for id, action in enumerate(action_n):
            if action == self.UE_act_space['Transmit']:
                transmitting = True
                transmitting_ue = id
                break
        ...
        if transmitting:
            # 如果有UE在传输，给该UE发送ACK
            bs_dcm[transmitting_ue] = self.BS_msg_space['ACK']
        elif sr_que:
            # 如果没有UE在传输且有SR请求，随机选择一个UE发送SG
            selected_ue = np.random.choice(sr_que)
            selected_ue_id = int(selected_ue.replace('UE', ''))
            bs_dcm[selected_ue_id] = self.BS_msg_space['SG']
        
        # 确定UE动作
        ue_act = []
        for id, UE in enumerate(self.UEs):
            if DCM[id] == self.BS_msg_space['SG'] and len(UE.buff) > 0:
                ue_act.append(self.UE_act_space['Transmit'])
            elif DCM[id] == self.BS_msg_space['ACK'] and len(UE.buff) > 0:
                ue_act.append(self.UE_act_space['Delete'])
            else:
                ue_act.append(self.UE_act_space['Do Nothing'])
        
        # 执行UE动作（确保只有一个UE在传输）
        self.data_channel.clear()  # 清空数据信道
        for id, UE in enumerate(self.UEs):
            if action_n[id] == self.UE_act_space['Transmit']:
                data = UE.transmit_SDU()
                if data is not None:
                    self.data_channel.append(data)
                    break  # 只允许一个UE传输数据
            elif action_n[id] == self.UE_act_space['Delete']:
                del_data = UE.delete_SDU()


        # 更新环境状态
        self.check_channel(error_del=0)
        self.UE_actions = action_n
        self.UE_msg = UCM
        self.BS_msg = DCM
        ...
        # 打印调试信息
        print('step:', self.step_count, 'UE_act:', action_n, 'datachannel:', self.data_channel)
        print('BS_received:', self.sdus_received)
...
if __name__ == "__main__":
    ...
    UCM = args.UCM
    DCM = args.DCM
    action_n = np.zeros(args.UE_num,dtype=np.int32)
    while env.done == False:
        a1,a2,a3 = env.cont_free_step(action_n,UCM,DCM)
        action_n = copy.deepcopy(a1)
        UCM = copy.deepcopy(a2)
        DCM = copy.deepcopy(a3)
        # action_n = np.random.randint(0,3,args.UE_num)
        # obs_n, reward_n, done_n, info_n = env.step(action_n)
        # print('traj_UE_obs:',env.trajact_UE_obs,'traj_UE_actions:',env.trajact_UE_actions,'traj_BS_obs:',env.trajact_BS_obs,'traj_BS_msg:',env.trajact_BS_msg,'traj_UE_msg:',env.trajact_UE_msg)