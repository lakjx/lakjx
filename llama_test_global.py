...
def arg_config():
    ...
    parser.add_argument('--UE_num', type=int, default=15)
...
def instruct_global(instructions):
    sen1 = f"Decide for BS and " + str(instructions['num_UEs']) + " UEs based on their state history."
    sen2=[]
    ...
    sen2 = re.sub(r"'(?!s\b)", "", '\n'.join(sen2))
    return sen1,sen2
def instruct_single(instructions):
    sen1,sen2=[],[]
...
def test_env(env,p_model,tokenizer):
    ...
    while env.done == False:
        ...
        sen1,sen2 = instruct_global(instructions)
        # sen1,sen2 = instruct_single(instructions)
...
if __name__ == "__main__":
    ...
    # 加载模型
    model, tokenizer = load_model(BASE_MODEL_PATH, ADAPTER_PATH_MERGE)
